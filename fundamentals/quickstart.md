...
SharpSync is a synchronization tool. It synchronizes data between 2 Data Sources. To do this, it uses the following to settings:

* Primary Component Identifier: The preferred property or meta data name to search
* Alternative Component Indentifier: The alternative property or meta data name to search if the primary returns an empty result.
...
Let's say we're working with part called '123'. Our primary identifier is `'PartNumber'` and our alternative identifier is `'name'.`

&#x20;The way SharpSync works is that it searches the source for the _Primary_ identifier (in this case the primary identifier is `PartNumber`) . So it  searches the source for `PartNumber` = `123`&#x20;
